...
const UserManagementDashboard = () => {
  ...
  const toggleUserStatus = async (user, isActive) => {
    console.log(isActive,'id', user);
    try {
      setActionLoadingState(`toggle_${user._id}`, true);
      // Fixed: Use 'deactivate' when user is active, 'reactivate' when user is inactive
      const endpoint = user?.isActive ? 'reactivate' : 'reactivate';
      const response = await fetch(`${API_BASE}/auth/users/${user?._id}/${endpoint}`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const Data = await response.json();
      ...
      setActionLoadingState(`toggle_${user._id}`, false);
    }
  };
